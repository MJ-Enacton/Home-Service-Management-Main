...
export function MyBookingsClient({ role, bookings }: MyBookingsClientProps) {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 pb-12 md:px-6">
      ...
      {/* Filter bar — compact, not blocky */}
      <div className="mb-6 flex flex-col gap-3 rounded-xl border bg-white p-3 dark:bg-zinc-900 sm:flex-row sm:items-end">
        {isCustomer && (
      ...
      {filteredBookings.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredBookings.map((booking) => (
            <div
              key={booking.id}
              className="rounded-xl border bg-white p-4 transition hover:border-zinc-300 dark:bg-zinc-900 dark:border-zinc-800 dark:hover:border-zinc-700"
            >
