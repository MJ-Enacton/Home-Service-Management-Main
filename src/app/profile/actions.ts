...
import { providerAvailability, providerProfiles, user } from "@/lib/db/schema";
...
async function requireProvider(): Promise<
  { ok: true; userId: string; name: string } | { ok: false; result: ActionResult }
> {
  ...
  return { ok: true, userId: session.user.id, name: currentUser.name };
}
...
export async function saveProviderProfile(
  ...
  const parsed = profileSchema.safeParse({
    bio: typeof (input as { bio?: unknown })?.bio === "string" ? (input as { bio: string }).bio : "",
    yearsExperience: (input as { yearsExperience?: unknown })?.yearsExperience,
    serviceAreas: Array.isArray((input as { serviceAreas?: unknown })?.serviceAreas)
  ...
  if ((input as SaveProviderProfileInput).removeAvatar) {
    ...
    const sizeBytes = Math.ceil(
      (avatarParsed.data.base64.length * 3) / 4,
    );
...
export async function saveAvailability(
  ...
  try {
    await db.delete(providerAvailability).where(
      eq(providerAvailability.providerId, guard.userId),
    );
