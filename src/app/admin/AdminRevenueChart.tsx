...
interface AdminRevenueChartProps {
  initialPoints: { date: string; label: string; value: number }[];
...
export function AdminRevenueChart({
  ...
  function syncUrl(nextRange: AdminRange, nextView: ChartType) {
    ...
    if (nextRange !== "week") params.set("revenueRange", nextRange);
    if (nextView !== "line") params.set("revenueView", nextView);
  ...
  function handleRange(next: AdminRange) {
    ...
    startTransition(async () => {
      const result = await getAdminSeries("revenue", next);
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {range === "week"
                ? "Daily gross · Mon–Sun"
                : "Daily gross · this month"}{" "}
              · {formatCents(total, { withCents: false })} total
            </p>
          </div>
        ...
        {isEmpty ? (
          <div className="flex h-65 flex-col items-center justify-center rounded-xl border border-dashed bg-zinc-50/60 px-6 text-center sm:h-75 dark:bg-zinc-900/40">
            ...
            <p className="mt-1 max-w-sm text-xs leading-relaxed text-muted-foreground">
              Paid bookings this {range === "week" ? "week" : "month"} will show
              up here.
            </p>
          </div>
        ) : (
